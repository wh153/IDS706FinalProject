...
def clean_data(json_gz_file)->pd.DataFrame:
    ...
    df_select = df_select.rename(columns = {'time':'ds', 'close':'y','volume':'volume', "high":"high","low":"low","open":"open"})
    return df_select
...
def predict(data):
    ...
    forecast = prophet_model.predict(prophet_model.make_future_dataframe(periods=30, freq="min"))
